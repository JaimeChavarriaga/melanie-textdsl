...
public class Ebnf2lml {
	...
	public static void main(String[] args) {
		try {
			if (args.length < 2) {
				System.out
						.println("Arguments not valid : {IN_model_path, OUT_model_path}.");
			} else {
				Ebnf2lml runner = new Ebnf2lml();
				runner.loadModels(args[0]);
				runner.doEbnf2lml(new NullProgressMonitor());
	 ...
	 * @generated
	 */
	public Ebnf2lml() throws IOException {
	...
	protected static URL getFileURL(String fileName) throws IOException {
		final URL fileURL;
		if (isEclipseRunning()) {
			URL resourceURL = Ebnf2lml.class.getResource(fileName);
			if (resourceURL != null) {
			...
			fileURL = Ebnf2lml.class.getResource(fileName);
		}
		if (fileURL == null) {
